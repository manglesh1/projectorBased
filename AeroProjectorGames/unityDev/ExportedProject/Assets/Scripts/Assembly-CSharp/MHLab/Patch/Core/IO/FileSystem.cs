...
namespace MHLab.Patch.Core.IO
{
	public class FileSystem : IFileSystem
	{
		...
		{
			return (FilePath)FilesManager.SanitizePath(PathsManager.Combine(PathsManager.GetSpecialPath(Environment.SpecialFolder.ApplicationData), folderName));
		}
		...
		{
			return (from stringPath in FilesManager.GetFiles(path.FullPath)
				select (FilePath)stringPath).ToArray();
		}
		...
		public void GetFilesInfo(FilePath path, out LocalFileInfo[] fileInfo, out Dictionary<string, LocalFileInfo> fileInfoMap)
		{
		...
		{
			return new FileStream(path.FullPath, fileMode, fileAccess, fileShare);
		}
		...
		public void UnlockFile(FilePath path)
		{
			...
			{
				FilePath filePath = (FilePath)GetTemporaryDeletingFileName(path);
