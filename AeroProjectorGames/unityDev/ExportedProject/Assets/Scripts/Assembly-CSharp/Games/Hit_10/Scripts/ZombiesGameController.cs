...
namespace Games.Hit_10.Scripts
{
	public class ZombiesGameController : MonoBehaviour
	{
		...
		[SerializeField]
		private ZombiesMultiDisplayScoringController multiDisplayController;
		...
		[SerializeField]
		private Sprite[] zombieTargetsArray;

		[SerializeField]
		private Sprite[] zombieTargetsToMissArray;
		...
		{
			targetsToHitArray = zombieTargetsArray;
			targetsToMissArray = zombieTargetsToMissArray;
		}
		...
		private IEnumerator HitAnimation(Transform TargetTransform, int roundScore)
		{
			...
			yield return LerpZombieAnimation(TargetTransform);
		...
		private IEnumerator LerpZombieAnimation(Transform TargetTransform)
		{
			ShowHideTargets(isActive: false, TargetTransform.gameObject);
			float currentTime = 0f;
			float duration = 6f;
			Vector3 newScale = TargetTransform.localScale;
			newScale += Vector3.one * 1000f;
			SpriteRenderer renderer = TargetTransform.gameObject.GetComponent<SpriteRenderer>();
			renderer.sortingLayerName = "Hit Effects";
			...
			while (currentTime < duration - animationCulling)
			{
				...
				Color color = new Color(1f, 1f, 1f, Mathf.Lerp(targetColor.a, newColor.a, currentTime / duration));
				targetColor = color;
				TargetTransform.gameObject.GetComponent<SpriteRenderer>().color = targetColor;
