...
namespace DigitalRuby.ThunderAndLightning
{
	public class LightningBolt
	{
		public class LineRendererMesh
		{
			...
			{
				int num = lineCount * 4;
				return vertices.Count + num <= 64999;
			}
		...
		public void Cleanup()
		{
			...
			List<LightningBoltSegmentGroup> list = groupCache;
			lock (list)
			{
		...
		private LineRendererMesh GetOrCreateLineRenderer()
		{
			...
			while (lineRendererCache.Count != 0)
			{
		...
		private IEnumerator GenerateParticleCoRoutine(ParticleSystem p, Vector3 pos, float delay)
		{
			...
			if (p.emission.burstCount > 0)
			{
				...
				int num = UnityEngine.Random.Range(array[0].minCount, array[0].maxCount + 1);
				p.Emit(num);
			}
			else
			{
