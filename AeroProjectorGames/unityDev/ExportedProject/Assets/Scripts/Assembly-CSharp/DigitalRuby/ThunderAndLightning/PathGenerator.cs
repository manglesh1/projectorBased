	...
	public static class PathGenerator
	{
		...
		public static float CreateCurve(ICollection<Vector3> path, Vector3 start, Vector3 end, Vector3 ctr1, Vector3 ctr2, int numberOfSegments, float startT)
		{
			...
			if (Is2D)
			{
				...
				{
					GetCurvePoint2D(ref start, ref end, ref ctr1, ref ctr2, num2, out var point);
					path.Add(point);
				}
			}
			else
			{
				for (num2 = startT; num2 <= 1f; num2 += num)
				{
					GetCurvePoint3D(ref start, ref end, ref ctr1, ref ctr2, num2, out var point2);
					path.Add(point2);
				}
			}
			return num2 - 1f;
		...
		public static bool CreateSpline(ICollection<Vector3> path, IList<Vector3> points, int numberOfSegments, bool closePath)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (Is2D)
				{
					...
					{
						GetSplinePoint2D(ref point, ref point2, ref point3, ref point4, num7, out var point5);
						path.Add(point5);
					}
				}
				else
				{
					for (num7 = num4; num7 <= 1f; num7 += num3)
					{
						GetSplinePoint3D(ref point, ref point2, ref point3, ref point4, num7, out var point6);
						path.Add(point6);
					}
				}
				num4 = num7 - 1f;
		...
		public static bool CreateSplineWithSegmentDistance(ICollection<Vector3> path, IList<Vector3> points, float distancePerSegment, bool closePath)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (Is2D)
				{
					...
					{
						GetSplinePoint2D(ref point, ref point2, ref point3, ref point4, num6, out var point5);
						path.Add(point5);
					}
				}
				else
				{
					float value2 = 1f / (Distance3D(ref point3, ref point2) / distancePerSegment);
					value2 = Mathf.Clamp(value2, 0.00390625f, 1f);
					for (float num7 = num3; num7 <= 1f; num7 += value2)
					{
						GetSplinePoint3D(ref point, ref point2, ref point3, ref point4, num7, out var point6);
						path.Add(point6);
					}
